public class C4Q7IntDeque {
    ...
    //생성자
    public C4Q7IntDeque(int capacity) {
        num = front = rear = 0;
        max = capacity;
        try {
            que = new int[max]; //덱본체용 배열을 생성
    ...
    public int enqueFront(int x) throws OverflowIntDequeException {
        ...
        num++;
        if (--front < 0) {
            front = max - 1; //front가 0보다 작을 경우 max-1 (마지막)인덱스로 front 변경
        }
        que[front] = x;
        return x;
    }
    ...
    public int enqueRear(int x) throws OverflowIntDequeException {
        ...
        num++;
        if (rear == max) {
            rear = 0;   //리어가 max일 경우 rear를 0으로 (링버퍼 형태)
        }
        return x;
    }
    ...
    public int dequeFront() throws EmptyIntDequeException {
        ...
        int x = que[front++];
        if (front == max) {
            front = 0; // front 가 max 와 같을 떄 front를 0으로
        }
        return x;
    }
    ...
    public int peekRear() throws EmptyIntDequeException {
        ...
        return que[rear == 0 ? max - 1 : rear - 1]; // 주의! rear는 -1하여 피크해야함!
    }
